...
void main()
{
	...
	vec3 sunDiffuse = vec3(1.0, 0.95, 0.9) * 1;
	//vec3 sunDiffuse = vec3(1.0, 0.1, 0.0) * 1;
	vec3 sunAmbient = vec3(0.2, 0.25, 0.3);
	//vec3 sunAmbient = vec3(0.2, 0.2, 0.25);
    ...
    //lray.direction =  rotMat * normalize(vec3(-(TexCoord.x - 0.5), (TexCoord.y - 0.5), focal_length / screensize.y / 2));


	FragColor =	texture(redraw_texture1, TexCoord);

	FragColor.rgb += sunDiffuse * pow(vec3(clamp(1 - acos(dot(lray.direction, sunDir)) * 5, 0.0, 1.0)) * 1.2, vec3(5)); //sun representation
	...
	float total_distance = sample_length;
...
//			}
//
//			lightlevel += sunDiffuse * total_brightness * phaseFunction(acos(dot(lray.direction, sunDir)), pow(u_scatterFactor, 1 / total_brightness));
//
//			resultColor += smokeColor * lightlevel * unscattered_left * (1 - pow(unscattered_left * (1 - local_density), 2)) * local_density;
...
//		}
		...
		if(loc.y > top_height && lray.direction.y > 0 || loc.y < bot_height && lray.direction.y < 0 || total_distance > max_distance || total_distance > pixelDepth)
			raymarch = false;
	...
	vec3 loc = vec3(TexCoord.x * 16, TexCoord.y * 9, u_scatterFactor) / 10;


	float Noise = texture(noiseTexture, vec3(loc.x * 1, loc.y * 1, u_scatterFactor)).r;
	float Noise1 = texture(noiseTexture, vec3(loc.x * 1, loc.y * 1, u_scatterFactor) * pow(lacunarity, 1)).r * pow(persistance, 1);
	float Noise2 = texture(noiseTexture, vec3(loc.x * 1, loc.y * 1, u_scatterFactor) * pow(lacunarity, 2)).r * pow(persistance, 2);
	float Noise3 = texture(noiseTexture, vec3(loc.x * 1, loc.y * 1, u_scatterFactor) * pow(lacunarity, 3)).r * pow(persistance, 3);


	float worleyNoise = (texture(worleyNoiseTexture,  vec3(loc.x * 2, loc.y * 2, u_scatterFactor) * pow(lacunarity, 0)).r * 2 - 1) * pow(persistance, 0);
	float worleyNoise1 = (texture(worleyNoiseTexture,  vec3(loc.x * 2, loc.y * 2, u_scatterFactor) * pow(lacunarity, 1)).r * 2 - 1) * pow(persistance, 1);
	float worleyNoise2 = (texture(worleyNoiseTexture,  vec3(loc.x * 2, loc.y * 2, u_scatterFactor) * pow(lacunarity, 2)).r * 2 - 1) * pow(persistance, 2);
	float worleyNoise3 = (texture(worleyNoiseTexture,  vec3(loc.x * 2, loc.y * 2, u_scatterFactor) * pow(lacunarity, 3)).r * 2 - 1) * pow(persistance, 3);
	float worleyNoise4 = (texture(worleyNoiseTexture,  vec3(loc.x * 2, loc.y * 2, u_scatterFactor) * pow(lacunarity, 4)).r * 2 - 1) * pow(persistance, 4);
	float worleyNoise5 = (texture(worleyNoiseTexture,  vec3(loc.x * 2, loc.y * 2, u_scatterFactor) * pow(lacunarity, 5)).r * 2 - 1) * pow(persistance, 5);
	float worleyNoise6 = (texture(worleyNoiseTexture,  vec3(loc.x * 2, loc.y * 2, u_scatterFactor) * pow(lacunarity, 6)).r * 2 - 1) * pow(persistance, 6);
	float amplitudePerlin = 1 + 0.5;
	float amplitudeWorley = 1 + 0.5 + 0.25 + 0.125 + 0.0625 + 0.03125 + 0.015625;
	float worleyTotal = (worleyNoise + worleyNoise1 + worleyNoise2 + worleyNoise3 + worleyNoise4 + worleyNoise5 + worleyNoise6) / amplitudeWorley;
	float perlinTotal = (Noise + Noise1) / amplitudePerlin; 
	Noise = texture(detailWorleyTexture, loc).r;
	//Noise = cnoise(loc * 32);
	//FragColor = vec4((vec3(worleyTotal + perlinTotal) / 4 + u_cloudCover - 0.5) / (u_cloudCover), 1.0);
	//FragColor = vec4(vec3(Noise), 1.0);
//	if(TexCoord.x > 0.5)
//	{
//		FragColor = vec4(vec3(texture(detailWorleyTexture, loc * 4).r), 1.0);
//	}
//	else
//	{
//		FragColor = vec4(vec3(texture(worleyNoiseTexture, loc * 2).r), 1.0);
//	}
	//FragColor = vec4(texture(perlinWorleyTexture, vec3(loc.xy, u_cloudCover)).r);
...
//	float Noise3 = cnoise(loc * pow(lacunarity, 3)) * pow(persistance, 3);
	//FragColor = vec4(lray.direction, 1.0);
}
