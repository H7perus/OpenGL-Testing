...
{
	btMatrix3x3 rotmat = plane.getWorldTransform().getBasis().inverse();

	btTransform localDir = plane.getWorldTransform();
	localDir(btVector3(1, 0, 0));
	btVector3 cgVel = rotmat * plane.getVelocityInLocalPoint(rotmat.inverse() * btVector3(0, 0, 0));
	btVector3 tailVel = rotmat * plane.getVelocityInLocalPoint(rotmat.inverse() * btVector3(0, 0, -5.6));
	...
	main_force *= 50 * pow(cgVel.length(), 2);
	...
	elev_force *= 5 * pow(tailVel.length(), 2);
	...
	rudder_force *= 5 * pow(tailVel.length(), 2);
	...
	btVector3 drag = -plane.getLinearVelocity().normalized() * pow(plane.getLinearVelocity().length(), 2);
	plane.applyForce(drag, btVector3(0, 0, 0));
}