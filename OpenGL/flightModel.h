...
void applyFlightModel(btRigidBody& plane)
{
	...
	btVector3 cgVel = rotmat * plane.getVelocityInLocalPoint(rotmat * btVector3(0, 0, 0));
	btVector3 tailVel = rotmat * plane.getVelocityInLocalPoint(rotmat * btVector3(0, 0, -5.6));
	...
	main_force *= 2000 * pow(cgVel.length(), 2);
	...
	elev_force *= 50 * pow(tailVel.length(), 2);
	...
	rudder_force *= 50 * pow(tailVel.length(), 2);
